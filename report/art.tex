...
and \texttt{key} w.r.t.\ the current depth. e.g., given a node with \texttt{prefix}
``\texttt{abbd}'', a search key ``\texttt{aaabbbccc}'' and depth $2$, then
\texttt{check\_prefix} returns $4$, since the $4$-th byte of \texttt{prefix}
...
node. \texttt{depth} is increased to account for the nodes merged due to 
vertical compression. We lookup the next child node, which is assigned to 
\texttt{cur}. If no such child exists, the search key does not exist and 
...
\begin{itemize}
  ...
  \item If \texttt{cur} has \textit{one sibling} (lines 24-48), vertical
    ...
    the concatenation of 1) \texttt{par}'s prefix, 2) \texttt{sibling}'s 
    partial key and 3) \texttt{sibling}'s prefix, to \texttt{sibling}'s prefix.

    Lines 40-45 perform cleanup operations, freeing memory previously allocated
    by \texttt{sibling}'s old prefix, \texttt{cur}, \texttt{cur}'s prefix,
...
estimate if ART outperforms RBT, or vice versa. HT, with an average case 
...
dataset, a dense dataset and a real-world dataset, called the Dell
dataset~\cite{wellenzohn2017wapi}, with sparse and dense regions which
will be described below in \Cref{sec:main-benchmark}. A dataset is dense 
(sparse) if many (few) keys share common prefixes. 
...
performance hit in the other microbenchmarks. We believe this happens because
during insertion and deletion, HT requires a table resize and full rehash
occasionally.
...
is based on Dell's website~\cite{wellenzohn2017wapi}, hence it's called Dell's
dataset. The keys are ordered in reverse level order and during each 
...
outperforms its competitors. As described in \Cref{sec:microbenchmarks}, ART,
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ybar,
      symbolic x coords={ART,RBT,HT},
      ylabel=Transactions/second {[M]},
      ymin=0,
      width=6cm,
      height=8cm,
      ]
      \addplot coordinates {(ART,0.9954678) (RBT,0.7042642) (HT,1.5987814)};
    \end{axis}
  \end{tikzpicture}
...
transactions is required~\cite{wellenzohn2017wapi}.
Our last experiment measures how often vertical compression is applied
when we store keys from the sparse, dense and the Dell dataset.
...
differs from the compressed path, expansion is applied, i.e., a new inner node
is created above the current node and the compressed paths are adjusted
accordingly. We only measure the number of vertical compressions during
deletion for brevity, as the number of times compression is applied during
deletion equals the number of times expansion is applied during insertion.
In other words, when inserting a set of keys and then consecutively deleting
them, expansions happen as often as compressions do.
...
for every deletion. A node with $s=8$ can have at most $254$ 
children removed before it is merged with its parent.
...
We plan to optimize ART's sequential performance with extensive CPU call-stack
profiling. Performance can also be improved by utilizing Single Instruction
