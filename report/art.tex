...
We refer to this heuristic as \textit{horizontal compression}.\footnote{The
authors refer to this technique as adaptive nodes.}

\begin{tikzpicture}
  \draw [help lines] (0,0) grid (4,2);
  \foreach \x/\key/\val in {0/65/a,1/82/b,2/84/c,3/0/0} {
    \node at (\x.5,2.3) {\x};
    % val
    \ifnum \key>0
      \node at (\x.5,1.5) {\key};
    ...
    % ptr
    \ifnum \key>0
      \node [circle,draw,scale=0.1] (\x-fr) at (\x.5,0.5) {};
      \node [regular polygon, regular polygon sides=3,draw, text width=0.1mm, inner sep=0pt] (\x-to) at (\x.5, -0.7) {\val};
      \draw [->] (\x-fr) -- (\x-to) {};
    \fi
  } 
\end{tikzpicture}
